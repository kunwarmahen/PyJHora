...
by the day's key highlights and — when the day carries them — the cautions the
engine flagged. Both the "send me a test now" endpoint and the background
scheduler call this, so the delivery logic lives in exactly one place.
...
async def _profile_block(user_id: str, profile: Dict[str, Any],
    ...
    return {
        ...
        "personal": personal,
        "changes": changes,
        # What to be careful of today — the engine's own caution lines, kept apart
        # from the highlights so the message can give them their own heading
        # instead of burying a hard transit in a list of neutral facts.
