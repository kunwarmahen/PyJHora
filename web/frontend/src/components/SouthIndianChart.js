 ...
 *   - `title` / `subtitle` for the heading and the caption in the grid's centre
 */
  ...
  return (
    <div className="chart-card">
      ...
      <div className="si-grid" ref={gridRef} role="img" aria-label={`${title} (${subtitle})`}>
          ...
          return (
            ...
            >
              ...
              <span className="si-sign" title={ln(RASI_NAMES[signNum - 1], "rasi")}>
                ...
                {labelParts.glyph && (
                  <span className="si-sign-glyph" style={{ color: rasiTattvaColor(signNum) }}>
                    {RASI_GLYPHS[signNum - 1]}
                  </span>
                )}
        ...
        {/* Caption. Unlike the North diamond — whose centre is the junction of
            the inner four houses — this style's inner 2x2 is purpose-built to
            hold the chart's name, and nothing else can occupy it. One line, so
            it labels the chart without becoming a second heading. It sits inside
            the grid, which is the export target, so a downloaded PNG says what
            it is. */}
        <div className="si-caption">
          <span>{subtitle ? `${title} · ${subtitle}` : title}</span>
        </div>
