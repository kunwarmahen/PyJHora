...
def _get_dhasa_dict(seed_star=mahadasa_seed_star):
    """
    Start with Ashwini nakshatra and take them in groups of 3 and 4 alternately.
    """
...
def get_dhasa_bhukthi(
    ...
    tribhagi_factor = 1.0
    effective_dhasa_cycles = dhasa_cycles

    if use_tribhagi_variation:
        tribhagi_factor = 1.0 / 3.0
        effective_dhasa_cycles = int(effective_dhasa_cycles / tribhagi_factor)
    ...
    for _ in range(effective_dhasa_cycles):
        for _ in range(len(dhasa_adhipathi_list)):
...
def shastihayani_immediate_children(
    ...
    """
    Shastihayani immediate children under the given parent span.
    ...
    if place is None:
        raise ValueError("place is required for exact Shastihayani child boundary calculation.")
...
def get_running_dhasa_for_given_date(
    ...
    """
    Shastihayani runner. Narrows Maha to requested depth.
