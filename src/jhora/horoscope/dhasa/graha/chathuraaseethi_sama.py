...
dhasa_adhipathi_list = {k: 12 for k in const.SUN_TO_SATURN}
...
def get_dhasa_bhukthi(
    ...
    """
    Returns a list of dhasa rows at selected depth L1..L6.
    ...
    def _children_of(parent_lord):
        """
        Returns child lords under parent_lord using the same Antara order rule.
        """
    ...
    def _recurse(level, parent_lord, parent_start_jd, parent_duration_years, prefix):
        """
        Recursive equal split of immediate parent.
        """
        ...
        if level < dhasa_level_index:
            for blord in bhukthis:
                ...
                jd_cursor += child_dur * year_duration
        else:
            for blord in bhukthis:
                start_tuple = utils.jd_to_gregorian(jd_cursor)
                retval.append([prefix + (blord,), start_tuple, child_dur])
                jd_cursor += child_dur * year_duration
    ...
    for _ in range(_dhasa_cycles):
        for _ in range(len(dhasa_adhipathi_list)):
            ...
            if dhasa_level_index == const.MAHA_DHASA_DEPTH.MAHA_DHASA_ONLY:
                ...
                start_jd += maha_dur * year_duration
            else:
                _recurse(
...
def nakshathra_dhasa_progression(
    ...
    # First get rasi position, then find varga division.
    planet_long = charts.get_chart_element_longitude(
        jd_at_dob,
        place,
        divisional_chart_factor=1,
...
def chathuraaseethi_sama_immediate_children(
    ...
    **kwargs,
    ...
    """
    Returns immediate children under a given parent span using same antara order
    and equal-split rule as get_dhasa_bhukthi().
    ...
    child_lords = list(_antardhasa(parent_lord, antardhasa_option))
...
def get_running_dhasa_for_given_date(
    ...
    **kwargs,
    ...
    """
    Narrow Maha -> ... -> target depth and return the full running ladder:
