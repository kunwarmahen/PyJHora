...
def _next_adhipati(lord):
    ...
    next_lord = keys[(current + 1) % len(keys)]
    return next_lord
...
def shodasottari_immediate_children(
    ...
    """
    Shodasottari immediate children under the given parent span.
    ...
    if place is None:
        raise ValueError("place is required for exact Shodasottari child boundary calculation.")
    ...
    child_lords = list(_antardhasa(parent_lord))
...
def get_running_dhasa_for_given_date(
    ...
    """
    Shodasottari runner. Narrows Maha to requested depth.
