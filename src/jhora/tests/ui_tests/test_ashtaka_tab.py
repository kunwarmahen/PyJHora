...
class TestAshtakaTab(BaseUITestCase):
    def test_ashtaka_tab_comprehensive(self):
        tab_name = "Ashtakavarga Tab: "
        ...
        chart_combo  = getattr(main_window, "_ashtaka_chart_combo", None)
        option_button = getattr(main_window, "_ashtaka_chart_option_button", None)
        info_label   = getattr(main_window, "_ashtaka_option_info_label", None)
        ...
        fallback_idx = getattr(horo_chart_tabs, "_ashtaka_varga_tab_start", 16)
        ...
        for i in range(total_charts):
            ...
            check(main_window._ashtaka_charts[0].isVisible(),f"{tab_name} charts are visible for {chart_name}")
            if i > 0: 
